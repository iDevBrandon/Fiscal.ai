...
function registeredUrl(year: number): string | undefined {
  ...
  return (typeof src === "function" ? src(year) : src[year]) ?? discoveredUrl(year);
}
...
function discoveredUrl(year: number): string | undefined {
  ...
  try {
    return (JSON.parse(readFileSync(file, "utf-8")) as Record<string, string>)[`${SLUG}:${year}`];
  } catch {
    return undefined;
  }
}
...
async function ensurePdf(year: number, url: string, refresh: boolean): Promise<Uint8Array> {
...
const HEADINGS: Record<Kind, string[]> = {
  income: ["income statement", "statement of operations", "statement of profit or loss", "profit and loss"],
  balance: ["statement of financial position", "balance sheet"],
  cashflow: ["statement of cash flows", "cash flow statement", "statements of cash flows"],
};
...
const CONFIRM: Record<Kind, string[]> = {
  income: ["net sales", "total revenue", "gross profit", "operating profit", "operating income", "profit before tax", "profit after tax", "net income", "cost of sales", "cost of revenue"],
  balance: ["total assets", "total equity", "total liabilities", "total current assets", "total non-current assets"],
  cashflow: ["operating activities", "investing activities", "financing activities", "net cash"],
};

function pageScore(pageLower: string, kind: Kind): number {
  let score = 0;
  for (const heading of HEADINGS[kind]) if (pageLower.includes(heading)) score += 5;
  for (const anchor of CONFIRM[kind]) {
    let i = pageLower.indexOf(anchor);
    while (i !== -1) {
      if (/\d{2,}/.test(pageLower.slice(i + anchor.length, i + anchor.length + 40))) {
        score += 1;
        break;
      }
...
function candidatePages(pages: string[]): { byKind: Record<Kind, number[]>; union: number[] } {
  const lower = pages.map((p) => p.toLowerCase());
  const byKind = { income: [], balance: [], cashflow: [] } as Record<Kind, number[]>;
...
async function classifyPages(client: OpenAI, pages: string[], candidateIdx: number[]): Promise<Record<Kind, number | null>> {
  const snippets = candidateIdx
    .map((i) => `Page ${i}: ${pages[i].replace(/\s+/g, " ").slice(0, 400)}`)
    .join("\n\n");

  const instructions =
    "You label pages of an annual report by which financial statement's MAIN TABLE they contain.\n" +
    "Choose the CONSOLIDATED GROUP statements. Treat parent-company standalone statements " +
    "(e.g. 'SAP SE ... German Commercial Code / HGB') as none.\n" +
    'Return ONLY JSON: { "income": <page index or null>, "balance": <page index or null>, "cashflow": <page index or null> }';
  ...
  const pick = (v: unknown) => (typeof v === "number" && candidateIdx.includes(v) ? v : null);
  return { income: pick(parsed.income), balance: pick(parsed.balance), cashflow: pick(parsed.cashflow) };
}
...
async function chat(client: OpenAI, params: ChatParams): Promise<string> {
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      console.warn(`    rate-limited (${status}) — retrying in ${waitMs / 1000}s…`);
...
async function extractStatement(client: OpenAI, pageText: string, kind: Kind): Promise<Statement> {
  const res = await chat(client, {
    model: MODEL,
    max_tokens: 4096,
    messages: [
      { role: "system", content: TRANSCRIBE },
      { role: "user", content: `This is the ${kind} statement. Return the JSON:\n\n${pageText.slice(0, 8000)}` },
    ],
  });
  const parsed = parseJson(res);
...
function parseJson(content: string): { periods?: string[]; rows?: Row[]; [k: string]: unknown } {
  ...
  const rows = labels.map((template) => {
    ...
    years.forEach((year, i) => {
      const reporting = newestFirst
        .map((r) => ({ year: r.year, value: valueFor(r.statement, template.label, year) }))
        .filter((r) => r.value !== null);
      if (reporting.length === 0) return values.push(null);
      values.push(reporting[0].value);
      if (reporting[0].year !== reporting[reporting.length - 1].year) restated.push(i);
    });
...
function valueFor(statement: Statement, label: string, year: number): number | null {
    ...
    if (row.kind === "line") {
      ...
      statement.periods.forEach((period, i) => {
        ...
        if (got != null && Math.abs(sum - got) > 1) {
          problems.push(`${row.label} @ ${period}: lines sum to ${Math.round(sum)}, row says ${got}`);
        }
      });
      lines = [];
    } else {
...
function writeDataset(slug: string, data: Record<Kind, Statement>, sources: { year: number; url: string }[]): string {
...
async function readReport(client: OpenAI, year: number, refresh: boolean): Promise<Report> {
  ...
  // LLM picks the consolidated page for each kind; fall back to the top keyword hit.
  let picks: Record<Kind, number | null> = { income: null, balance: null, cashflow: null };
  if (union.length) picks = await classifyPages(client, pages, union);
  for (const kind of KINDS) if (picks[kind] === null) picks[kind] = byKind[kind][0] ?? null;
  ...
  const statement = async (kind: Kind): Promise<Statement> => {
    ...
    console.log(`    ${kind}: page ${idx}`);
    return extractStatement(client, pages[idx], kind);
  };
...
async function main() {
  ...
  MODEL = process.env.OPENAI_MODEL || (process.env.CLOUDFLARE_ACCOUNT_ID ? "@cf/meta/llama-3.3-70b-instruct-fp8-fast" : "gpt-4o");
  ...
  if (COMPANIES[arg]) {
    ...
    SLUG = arg.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  ...
  const years =
    ...
    (COMPANY.years.length ? COMPANY.years : fail(`Pass --years for '${arg}', e.g. --years 2024-2024`));
  const refresh = flags.has("--refresh");
  console.log(`=== ${COMPANY.name} — years ${years.at(0)}–${years.at(-1)} ===`);

  const reports = [];
  for (const year of [...years].sort((a, b) => b - a)) reports.push(await readReport(client, year, refresh));

  // Provenance: best-effort URL (registry/cache; populated by discovery if it ran).
  const sources = [...years].sort((a, b) => b - a).map((year) => ({ year, url: registeredUrl(year) ?? "" }));

  const data: Record<Kind, Statement> = {
    income: compile(reports.map((r) => ({ year: r.year, statement: r.income }))),
    balance: compile(reports.map((r) => ({ year: r.year, statement: r.balance }))),
    cashflow: compile(reports.map((r) => ({ year: r.year, statement: r.cashflow }))),
  };

  for (const kind of KINDS) {
    const problems = reconcile(data[kind]);
    console.log(`  ${kind}: ${data[kind].periods.length} periods, ${data[kind].rows.length} rows, ${problems.length} check issue(s)`);
...
function makeClient(): OpenAI {
  ...
  if (account && token) {
    return new OpenAI({ apiKey: token, baseURL: `https://api.cloudflare.com/client/v4/accounts/${account}/ai/v1` });
  }
  if (process.env.OPENAI_API_KEY) {
    return new OpenAI({ apiKey: process.env.OPENAI_API_KEY, baseURL: process.env.OPENAI_BASE_URL });
  }
  fail("Set CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN (or OPENAI_API_KEY) in your environment");
}
...
function loadEnv(path = join(HERE, "..", ".env")): void {
  ...
  for (const line of readFileSync(path, "utf-8").split("\n")) {
    ...
    if (!(key in process.env)) process.env[key] = t.slice(i + 1).trim().replace(/^["']|["']$/g, "");
  }
}
