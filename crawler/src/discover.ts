...
let MODEL = "gpt-4o";
const USER_AGENT = "Mozilla/5.0 (compatible; fiscal-ai-discovery/1.0; +https://github.com/)";
...
async function fetchPage(url: string): Promise<{ url: string; title: string; links: { text: string; href: string }[] }> {
...
async function listPdfLinks(url: string): Promise<{ url: string; pdfLinks: { text: string; href: string }[] }> {
  const { links } = await fetchPage(url);
  return { url, pdfLinks: links.filter((l) => l.href.toLowerCase().split("?")[0].endsWith(".pdf")) };
}

function saveUrl(slug: string, year: number, url: string): { saved: boolean; slug: string; year: number; url: string } {
  mkdirSync(CACHE_DIR, { recursive: true });
  const db = existsSync(DISCOVERED) ? JSON.parse(readFileSync(DISCOVERED, "utf-8")) : {};
  db[`${slug}:${year}`] = url;
...
export async function discoverPdfUrl(company: string, year: number, slug: string): Promise<string | null> {
  ...
  const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
    {
      role: "system",
      content:
        "You find the direct PDF URL of a company's ANNUAL REPORT for a given year.\n" +
        ...
        "- save_pdf_url(url): save the final annual-report PDF URL and finish\n" +
        "Prefer the FULL annual report PDF (US GAAP or IFRS) over quarterly or partial files.\n" +
        "Reply with ONLY a JSON object, no prose, e.g.\n" +
        '{"tool":"web_search","args":{"query":"Company annual report 2024 pdf"}}\n' +
        'When you have the direct .pdf URL, reply {"tool":"save_pdf_url","args":{"url":"https://…"}}.',
    },
    { role: "user", content: `Company: ${company}\nYear: ${year}\nFind the annual report PDF URL. Reply with your first action as JSON.` },
  ];
  ...
  for (let step = 0; step < 12; step++) {
    ...
    if (!action) {
      messages.push({ role: "user", content: 'Reply with ONLY a JSON action, e.g. {"tool":"web_search","args":{"query":"…"}}.' });
      continue;
    }
    console.error(`→ ${action.tool}(${JSON.stringify(action.args).slice(0, 90)})`);
...
function resolveModel(): string {
  return (
    ...
    (process.env.CLOUDFLARE_ACCOUNT_ID ? "@cf/meta/llama-3.3-70b-instruct-fp8-fast" : "gpt-4o")
  );
}
...
async function chatWithBackoff(client: OpenAI, params: OpenAI.Chat.Completions.ChatCompletionCreateParamsNonStreaming) {
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      console.error(`    rate-limited (${status}) — retrying in ${waitMs / 1000}s…`);
...
async function main() {
  ...
  if (!company) fail('Usage: npm run discover -- --company "SAP SE" --year 2025 [--slug sap]');
  const year = Number(argValue("--year") ?? new Date().getFullYear() - 1);
  const slug = argValue("--slug") ?? company.toLowerCase().split(/\s+/)[0].replace(/[^a-z0-9]/g, "");

  console.log(`Discovering ${company} ${year}…`);
  const url = await discoverPdfUrl(company, year, slug);
  if (!url) fail("Could not find a PDF URL. Try a different --company phrasing, or the page may be JS-rendered.");
}
